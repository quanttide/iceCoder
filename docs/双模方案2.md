# iceCoder 双模自适应运行时监管系统（V1.3.6）

## 状态

工程实现规格版（Implementation Spec）· **实现决议与附录 A/B 已冻结**（2026-05）  
**V1.3.1 增补（2026-05）：** §2.6–§2.7 架构公理与干预分层；§14.0 `ToolGate` / `CorrectionPort`；§19.4 / §19.6 全相位互斥；§21 Phase 0b；§23 竞品对比与自由度轴线；附录 B 门禁验收项。  
**V1.3.3（2026-05）：** §2.8 Execution Mode Boundary、`ModeDecisionEngine` / `TaskRiskClassifier`、附录 C。  
**V1.3.4（2026-05）：** §3.2 Global Mode Policy、公理 I6。  
**V1.3.5（2026-05）：** §2.8.8–§2.8.11 signal 优先级、`enteredBy`、fail-safe、Degraded Execution。  
...
| 排期与验收 | §21–§22、附录 B | 当前至 **5D**；§22 外链 [`运行时后续优化.md`](./运行时后续优化.md) |
| 竞品与定位 | §23–§24 | 相对 CC / Codex 的轴线与场景对比 |

> **冲突时以 §14–§21、§23–§24、附录 A/B/C 为准。** §22 仅指向后续文档；benchmark 指标以 [`运行时后续优化.md`](./运行时后续优化.md) 为准。§6、§12 等早期数值若与 §17 不一致，**以 §17 参数矩阵为准**。
...
**5E / Learning：** 仅可产出 **阈值建议** 或 shadow 报表；**不得**成为 `executionMode` 主决策层（见 [`运行时后续优化.md`](./运行时后续优化.md) §4）。
...
**当前范围：** Phase 0 / 0b → **5D** 为必做里程碑。**Phase 5E**（Learning、benchmark、验收自动化）见 **[`运行时后续优化.md`](./运行时后续优化.md)**，本期不做。

## Phase 0（约 3–5 天）

- [ ] `src/types/supervisor.ts`（**附录 A** 落盘）
- [ ] `ICE_SUPERVISOR_*` + `data/supervisor-config.json`（**§15** 全字段注释；`supervisor-config.example.json` 可选）
- [ ] `HarnessStepEvent`：`supervisor_takeover` / `supervisor_handoff` / `supervisor_fail`；**`execution_mode_enter` / `execution_mode_exit`**（含 `enteredBy`，§2.8.9）
...
- [ ] README 环境变量表补充 `ICE_SUPERVISOR_*`（与 **§15.1** 一致；标注规划中）
...
- [ ] 测试：`block` 时对应 `toolCallId` 未执行；`CorrectionPort` 为 takeover 段唯一 C 源（见附录 B）

## Phase 0c（约 2–3 天，**5A 前置必做**）

> 落实 **§2.8 · I5 · §14.4**；与 Phase 0b 可并行，但 **不得晚于 5A**。

- [ ] `src/harness/supervisor/mode-decision-engine.ts`：`evaluate` / `submitSignal` / mode lock
- [ ] `src/harness/supervisor/task-risk-classifier.ts`：L0/L1/L2 **运行态**分级
- [ ] `src/harness/supervisor/execution-mode-constraints.ts`：`applyExecutionModeConstraints`
- [ ] `harness.ts`：`prepareHarnessRound` 与 `callHarnessLlm` 之间接入（**附录 C.2**）
- [ ] `supervisor-config.json` → `executionMode` 段（**§15.5**）
- [ ] 测试：`GraphExecutor` 等 **不得**直接写 `executionMode`；Forced 进入后 **2 轮**内不可 exit（附录 B）

## Phase 5A（约 1.5–2 周）

- `ModeController`、`TaskDomainClassifier`、`TaskRiskClassifier`
...
|------|----------------|
| 1 人熟仓库 | 约 12–15 周 |
| 2 人（runtime + graph/测试） | 约 6–9 周 |
| 调参与真实任务回归 | 另计 2–4 周 |

**5E 工期**（Learning / benchmark）：见 [`运行时后续优化.md`](./运行时后续优化.md) §6。

---

# 22. benchmark 与后续优化

Phase 5E 及 benchmark 指标、Learning 层、eval 扩展已拆至独立文档：

**[`运行时后续优化.md`](./运行时后续优化.md)**

5D 阶段以 **附录 B 人工勾选** + `ICE_SUPERVISOR_SHADOW` 对照为主；不要求 benchmark CSV 或 Learning 闭环。
...
| 维度 | Claude Code | Codex CLI | iceCoder V1.3.2（含 §2.6 / §14.0） |
|------|-------------|-----------|--------------------------------------|
| 默认姿态 | 模型全程决策 | 模型循环 + 工具沙箱 | **信任优先**：free 段接近 CC；**必要时**接管 |
| 规划 | 自发 TODO | 模型自规划 | free：模型；takeover：反构图 / 模板图 |
| 纠偏 | 对话自纠正 | 对话 + 云端状态 | free：Observer 信号 → takeover；takeover：**CorrectionPort 单出口** |
| 工具约束 | 软（hooks） | **白名单 / 沙箱** | **ToolGate**：block = 不执行（**I2**） |
| 何时变严 | 基本不变 | 基本不变 | 域 + 风险 + 异常 → `supervisorPhase=takeover` |
...
高自由度 ◄────────────────────────────────────────────────────► 强规则
 ...
 Claude Code ── Codex ── iceCoder(adaptive·free) ── Trae SOLO ── iceCoder(当前实现*)
                              │                              │
                              │    *迁移前：首轮建图+软 block  │
                              └──► iceCoder(adaptive·takeover) ──► strict
```
...
| iceCoder **V1.3.2 adaptive·free** | 左中 | 对齐 CC 主路径；多 Observer，少 C 类 inject |
| Trae SOLO | 正中偏右 | **人批 Plan**；执行期未必连续合约 |
| iceCoder **当前代码**（迁移前） | 中右 | 关键 intent 首轮建图 + 多路 inject + 软 block |
| iceCoder **adaptive·takeover** | 右 | ToolGate + 单出口纠偏 + 恢复预算 |
| iceCoder **strict** | 最右 | 关键域首轮全图 + 全程 Gate |

V1.3.2 实施后：**主路径左移对齐 CC，翻车段右移且变硬**。

## 23.3 典型场景对比

| 场景 | CC / Codex | iceCoder V1.3.2 |
|------|------------|-----------------|
| 解释代码 | 自然、快 | **相当**（非关键域） |
...
## 23.4 预期 benchmark 对比（机制分析，待 [`运行时后续优化.md`](./运行时后续优化.md) 实测）
...
| Phase 5E Learning | 仅只读 timeline / 建议阈值；**禁止**写 `executionMode`（§2.8.7） |
...
| 6 | Phase 0b ToolGate 与 Forced 联调 | block 在 forced 生效、free 不检查 |
...
> **Phase 5E（可选）：** benchmark CSV、验收自动化等见 [`运行时后续优化.md`](./运行时后续优化.md) §2–§3；**不计入 5D 完成条件**。
