...
**指标：** `task_success_rate`、`tool_call_rate`、`first_tool_latency`、`no_tool_final_rate`、`verification_rate`、`repeat_failure_rate`、`memory_interference_rate`、`tokens_per_successful_task`、`compaction_saved_tokens`。

**TaskGraph 评测（独立）：** `npx tsx scripts/eval-runner.ts` — 图完成度 / 节点评分（benchmark fixture）。

后续：CI 门禁、遥测趋势对接 — 见 [`docs/nextWork.md`](./nextWork.md) §3–§4。
