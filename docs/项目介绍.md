...
**测试基线（2026-06-01）：** `test/**/*.test.ts` 共 **173** 个文件、**1844** 条用例（`npm test`，~39s）。完整分层与手工结果见下文 **§10 测试与验收**。
...
- [`docs/test.md`](./test.md) — **双模完整测试手册**（1844 自动化 + 15 项 Web 手工场景）
...
**基线（2026-06-01）：** **173** 测试文件 · **1844** 用例 · **~39s** · `npx tsc --noEmit` 0 error

| 目录 | 文件 | 说明 |
|------|------|------|
| `test/harness/` | **90** | Harness 主循环、压缩、checkpoint、**supervisor 全链路**、Shell 双轨 |
| `test/memory/` | **20** | 文件化记忆召回 / 提取 / Dream / 淘汰 |
| `test/web/` | **19** | API、sessions、setup 门禁、supervisor-events、chat-ws |
