...
| 功能 | 规格 | 当前 |
|------|------|------|
| 连续工具失败 → takeover（非 free 段重复 inject） | §19.6、I4 | ⚠️ L2-3/L2-5 接管路径完成；free 段 `harness-resilience` 收口待 L2-6 |
...
| 恢复预算耗尽 → `user_checkpoint` | §8.8、§13 | ⚠️ M9 已落 `RecoveryBudgetManager`；耗尽返回 `fail{checkpoint}` 决策；Harness `stopReason: 'user_checkpoint'` 串联待 L2-6 |
