...
**与 Gate 的关系（2026-05-28 起）**：每次 `edit_file` 后须 **`read_file` 同路径** 才能通过写后读 Gate；本场景 **故意不跑对测试**，Cycle 里仍必须 read，否则会在 `model_done` 前被 Verification Gate 拦住，**到不了 L2 预算耗尽**。
...
| Gate 注入但无 L2 | 只 edit 未 read；或 Cycle 次数不足 |
...
4) 若收到 [System] 写后读 / file_info / read_file 提示，必须先 read_file 再继续 edit 或 vitest。
...
Cycle = 顺序执行：
  ...
  (b) read_file test/core/types.test.ts — 写后读确认（Gate 硬要求）
  (c) run_command: npx vitest run test/core/types.test.ts — 预期失败，立刻进入下一 Cycle

优先在同一轮 LLM 回复内连续发出 (a)(b)(c)；若单轮只能 1 个工具，下一轮继续**当前 Cycle**，不得跳过 read 或 vitest，不得跳 Cycle。
