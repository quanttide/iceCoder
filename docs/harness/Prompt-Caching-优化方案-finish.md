...
> 版本：2026-06-02（实测归档 2026-06-08）  
> 状态：**已实施**（阶段 1～3 已落地，`feat-lb-cache`）  
...
#### 10.5.1 实测结果（2026-06-08，`feat-lb-cache`，DeepSeek-V4-Flash）

**环境：** iceCoder Harness + 本仓库 `docs/harness/cache-hit-bench-prompts.md` 提示词集（P1～P4 组合压测）；模型 **`deepseek-v4-flash`**；同一会话多轮工具调用。
...
| **命中率** = 命中 / (命中+未命中) | **83.76%** | **83.64%** | **84.25%** | **+0.61 pp** |
...
| 输出 | 79,226 | — |
| 总输入 | 4,887,754 | 100% |
| **命中率** | 4,118,144 / 4,887,754 | **84.25%** |

| 指标 | 实测值 | 目标区间 | 结论 |
|------|--------|----------|------|
| 稳态命中率（n=3） | **~83.9%**（极差 0.61 pp） | 70%–85% | **区间上沿 ✅** |
| 未命中占比随总量增长 | ~15.8% 保持稳定 | 15%–30% | **达标 ✅** |
| 相对改造前预估（25%–45%） | **+38 ~ +59 pp** | +30 ~ +50 pp | **达成 ✅** |
| DeepSeek 控制台实付 | **¥1.01** | — | 第 3 次运行账单 |
| 相对「全未命中」理论价（同口径估算） | 节省 **~79.5%** | ≥ 40% | **达成 ✅** |

**对照拟合：** 三次实测均落在 §10.3 中等任务预测 **75%～85%** 的**上沿**，与点估计 ~82% 偏差 < 2.5 pp。4.9M 总 token / 4.9M 输入下命中率极差 < 0.7 pp，说明封存裁剪 + 易变块 ephemeral 分离已进入**生产级稳态**；剩余 ~16% miss 主要来自每轮末尾增量（user/assistant/tool）及偶发 compact 分段，继续优化边际收益递减。

**说明：** `taskSwitchInjected` 按 §5 阶段 2 设计保留在主历史、每会话最多写入一次，触发后前缀稳定，**非**命中率拖累项（见 `cache-hit-bench-prompts.md` §3.2 观测说明）。
