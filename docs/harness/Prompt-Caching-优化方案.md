...
- 将「三层提示词结构」收敛为本项目可落地的 **双轨上下文 + 封存裁剪** 方案。
...
| `applySubAgentResultRetention` 修改工具正文 | `harness-round-prep.ts`（发送副本） | 与主历史共享引用时会污染持久历史 |
...
| `reasoningContent` 写入 assistant 并回传 API | `openai-adapter.ts` | DeepSeek 思考模式 **必须**；无法「不发服务器」 |

### 3.3 第三层（易失草稿）在本项目的取舍

- **不可行**：思考模式下省略 `reasoning_content`（多轮工具调用会返回 400）。
- **可行**：运行时状态 / 工作区锚点 / 图节点 / 后台摘要等 **易变块** 不进主历史，仅在 **发送视图** 注入。
...
```mermaid
flowchart TB
  subgraph canonical [主历史 — state.messages]
    SYS[系统提示 静态]
    USER[用户对话 + 记忆召回等 只追加]
    ASST[助手 + 工具 只追加]
  end

  subgraph sendview [发送视图 — 每轮 LLM 前临时构建]
    CANON[拷贝主历史]
    NORM[规范化 / 终态整理]
...
| 轨道 | 职责 | 规则 |
|------|------|------|
| **主历史** | 真相源（持久化、压缩、UI） | 只追加；**禁止**删除或改写旧正文 |
| **发送视图** | 发给模型的消息列表 | 裁剪、运行时注入、封存 **仅在此构建** |
...
| 易失草稿 | **半采纳**：易变块不进主历史；`reasoning_content` 仍进 API |
...
新增 `buildMessagesForLlm(主历史, 易变注入)`（建议放在 `context-assembler.ts` 或新建 `harness-api-messages.ts`）：
...
`harness-round-prep.ts` 末尾改为调用 `buildMessagesForLlm`，不再对主历史与发送副本各裁剪一次。

### 阶段 2 — 中等投入产出比（约 2 个文件，约 0.5 人日）

将以下逻辑 **从 `state.messages` 挪到发送视图末尾追加**（不再删除旧块）：

| 模块 | 现状 | 改法 |
|------|------|------|
| `upsertRuntimeContextMessage` | 删旧 + 写入主历史 | 仅追加到发送视图 |
| `upsertWorkspaceAnchorMessage` | 同上 | 同上；内容哈希不变时可跳过 |
| `takeBgStatusForInjection` | 写入主历史 | 发送视图 |
| `graphExecutor.getCurrentNodeContext()` | 写入主历史 | 发送视图 |
| 首轮 `formatToolPlan` | 写入主历史 | 发送视图 |
...
| 思维链不进 API | DeepSeek 思考模式 + 工具协议要求 |
...
| `src/harness/harness-round-prep.ts` | 1～2 | 删主历史 budget；发送视图 |
| `src/harness/harness-compaction.ts` | 1 | 删压缩前 budget |
| `src/harness/context-assembler.ts` 或新建 `harness-api-messages.ts` | 1～2 | `buildMessagesForLlm` |
| `src/harness/harness-runtime-inject.ts` | 2 | 易变注入 |
| `src/harness/workspace-anchor.ts` | 2 | 易变注入 |
...
4. DeepSeek 思考模式 + 多轮 tool **无 400**（`reasoning_content` 仍完整回传）；
...
| **阶段 1** | **低～中** | 删 2 处 budget + 封存字段 + 单一发送视图入口；约 **1 人日** |
| **阶段 2** | **中** | 多处注入从主历史迁到发送视图；需回归压缩、记忆、任务图；约 **0.5 人日** |
...
**与「理想三层全量落地」对比：** 全量只追加 + 禁止压缩 + 思维链不发 API，在思考模式下 **不可行**；本方案是 **最小可行折中**，实施难度约为全量方案的 **30%～40%**。
...
| **模型行为** | **基本无变化** | 发送视图仍注入运行时状态、记忆、图节点；模型可见信息与现网一致 |
...
| **调试可读性** | **略升** | 主历史与「发给 API 的视图」分离后，日志可区分二者，便于排查缓存未命中 |
...
- 硬压缩 / 微压缩 → 分段清零；
- `reasoning_content` 随轮增长（思考模式必要成本）；
...
| 4 | 空白符规范化（`\r\n`、trim 等） | **不实施** | 需动发送视图/规范化链路，与 tool 结果语义边界难界定；**回归风险 > 预估 +1～3 pp 收益** |
| 5 | 发送视图内重排消息块 | **不实施** | 对话须保持 assistant↔tool 时序，**不可**对调早期/近期历史；仅重排尾部亦与 Phase 2 易变注入 **叠加复杂** |
...
- **#4**：空白符差异真实存在，但需在发送视图统一处理，易与 tool 输出边界 case 纠缠。
- **#5**：只能重排 ephemeral 尾部，收益小且与 Phase 2 发送视图 **重复建设**。
...
| 日期 | 说明 |
|------|------|
| 2026-06-02 | 初版：双轨上下文 + 封存裁剪方案；附录含实施难度、用户体验、命中率、费用估算 |
