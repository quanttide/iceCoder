...
import { MAX_FACTS_PER_FILE } from '../../../src/memory/file-memory/memory-config.js';
...
describe('buildIndex — 基本提取', () => {
  it('从 contentPreview 提取多行 facts', async () => {
    const mem = makeHeader({
      contentPreview: '用户偏好 TypeScript 语言\n用户使用 React 框架开发\n用户习惯用 Vitest 做测试',
    });
    const facts = await index.buildIndex([mem]);
  ...
  it('从 fullContents 提取 facts（跳过 frontmatter）', async () => {
...
在创业公司工作多年`);
    const facts = await index.buildIndex([mem], full);
    expect(facts.length).toBe(3);
    expect(facts[0].factText).toBe('用户是前端开发者角色');
  });

  it('fullContents 优先于 contentPreview', async () => {
    const mem = makeHeader({
      filePath: '/mem/b.md',
...
describe('buildIndex — 过滤和分割', () => {
  it('过滤短于 MIN_FACT_LENGTH(6) 的行', async () => {
    const mem = makeHeader({
      contentPreview: '好的\nok\n用户偏好 TypeScript 语言\nyes\n项目使用 React 框架',
    });
    const facts = await index.buildIndex([mem]);
  ...
  it(`每个文件最多 MAX_FACTS_PER_FILE(${MAX_FACTS_PER_FILE}) 条`, async () => {
    const lines = Array.from({ length: MAX_FACTS_PER_FILE + 10 }, (_, i) => `这是第 ${String(i + 1).padStart(2, '0')} 条事实信息内容`).join('\n');
    const mem = makeHeader({ contentPreview: lines });
    const facts = await index.buildIndex([mem]);
    expect(facts.length).toBe(MAX_FACTS_PER_FILE);
  });

  it('空内容返回空 facts', async () => {
    const mem = makeHeader({ contentPreview: '' });
    expect(await index.buildIndex([mem])).toEqual([]);
  });

  it('只有空行和格式标记的内容返回空', async () => {
    const mem = makeHeader({ contentPreview: '\n\n---\n\n' });
    expect(await index.buildIndex([mem])).toEqual([]);
  });
});
...
describe('buildIndex — 元数据继承', () => {
  ...
  it('type 为 undefined 时正确继承', async () => {
...
describe('缓存机制', () => {
  ...
  it('不同 filePath 独立缓存', async () => {
    ...
    await index.buildIndex([memA]);
    await index.buildIndex([memB]);
    expect(index.getCacheStats().fileCount).toBe(2);
    expect(index.getCacheStats().totalFacts).toBe(2);
  });

  it('clearCache 清除所有缓存', async () => {
    await index.buildIndex([makeHeader({ contentPreview: '用户偏好 TypeScript 语言' })]);
    expect(index.getCacheStats().fileCount).toBe(1);
    index.clearCache();
...
describe('rankFacts', () => {
  it('英文关键词匹配并排序', async () => {
    const facts = await index.buildIndex([makeHeader({
  ...
  it('无匹配时返回空数组', async () => {
    const facts = await index.buildIndex([makeHeader({ contentPreview: '用户偏好 React 框架开发' })]);
    const ranked = index.rankFacts('xyzzy_no_match_at_all', facts, 5);
    expect(ranked).toEqual([]);
  });

  it('限制 maxResults', async () => {
    const lines = Array.from({ length: 10 }, (_, i) => `TypeScript 相关事实内容 ${i}`).join('\n');
    const facts = await index.buildIndex([makeHeader({ contentPreview: lines })]);
    const ranked = index.rankFacts('TypeScript', facts, 3);
...
describe('getTopFactsForFile', () => {
  it('有查询时返回精排后的 top facts', async () => {
    const mem = makeHeader({
      filePath: '/mem/user.md',
      contentPreview: '偏好 TypeScript 语言开发\n使用 React 框架构建\n习惯 Vitest 做测试',
    });
    await index.buildIndex([mem]);
...
describe('全局单例', () => {
  it('getFactIndex 返回同一个实例', async () => {
    const a = getFactIndex();
    const b = getFactIndex();
    expect(a).toBe(b);
  });

  it('resetFactIndex 后返回新实例', async () => {
    const a = getFactIndex();
    resetFactIndex();
    const b = getFactIndex();
    expect(a).not.toBe(b);
  ...
  it('resetFactIndex 清除旧实例的缓存', async () => {
    const inst = getFactIndex();
    await inst.buildIndex([makeHeader({ contentPreview: '用户偏好 TypeScript 语言' })]);
...
describe('多文件场景', () => {
  it('跨文件构建索引', async () => {
    ...
    const facts = await index.buildIndex(mems);
    expect(facts.length).toBe(2);
  ...
  it('部分文件命中缓存，部分重新提取', async () => {
    const memA = makeHeader({ filePath: '/mem/a.md', mtimeMs: 1000, contentPreview: '事实 A 的内容信息' });
    await index.buildIndex([memA]); // 缓存 A

    const memB = makeHeader({ filePath: '/mem/b.md', mtimeMs: 2000, contentPreview: '事实 B 的内容信息' });
    const facts = await index.buildIndex([memA, memB]); // A 命中缓存，B 新建
    expect(facts.length).toBe(2);
    expect(index.getCacheStats().fileCount).toBe(2);
  });
});
