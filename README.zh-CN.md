...
| **双模 Supervisor** | **L2 基本可用** — L1/L2 已接入；Web 手工 15 场景见 **§10.3**；规格 [`docs/双模方案2.md`](./docs/双模方案2.md) |
...
**测试基线（2026-05-22）：** `test/**/*.test.ts` 共 **93** 个文件、**1165** 条用例（`npm test`）。完整分层与手工结果见下文 **§10 测试与验收**。
...
规格权威：[`docs/双模方案2.md`](./docs/双模方案2.md) · 手工手册：[`docs/test.md`](./docs/test.md) · 流程图：[`docs/双模 L2 流程图.md`](./docs/双模%20L2%20流程图.md)
...
- **环境变量**：见 [`docs/环境变量.md`](./docs/环境变量.md) §4
...
常用命令：`npm test -- test/memory/` · 设计说明：[`docs/requirement/记忆系统调整-finish.md`](./docs/requirement/记忆系统调整-finish.md)
...
- [`docs/test.md`](./docs/test.md) — **双模完整测试手册**（1165 自动化 + 15 项 Web 手工场景）
...
> 手工步骤与可复制提示词：[`docs/test.md`](./docs/test.md) · 双模 e2e：`test/e2e/dual-mode-scenarios.test.ts`

### 10.1 自动化测试（Vitest）

**基线（2026-05-22）：** **93** 测试文件 · **1165** 用例 · `npx tsc --noEmit` 0 error

| 目录 | 文件 | 用例（约） | 说明 |
|------|------|-----------|------|
| `test/harness/` | **45** | **~396** | Harness 主循环、压缩、checkpoint、**supervisor 全链路** |
| `test/memory/` | **20** | **~391** | 文件化记忆召回 / 提取 / Dream / 淘汰 |
| `test/`（TaskGraph 等） | **7** | **~166** | TaskGraph 构建、执行、持久化、review |
| `test/llm/` | 4 | ~34 | 适配器、token 计数 |
| `test/web/` | 5 | ~26 | API、supervisor-events 报告 |
| `test/public/` | 2 | **12** | **冰豆**色板与表情周期 |
| `test/e2e/` | **1** | **7** | **双模六场景** A–F（见下表） |
| 其他 | 9 | ~139 | parser、cli、core、prompts、config |
...
环境：Web 聊天 · Windows · 模型 z-ai/glm-5.1 / minimax-m2.5 · 详见 [`docs/test.md`](./docs/test.md) §10–§11
