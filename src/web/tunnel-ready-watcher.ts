 ...
 *
 * 【过渡方案】有固定公网 IP 且不再使用 Quick Tunnel 时，可设 ICE_TUNNEL_WS_NOTIFY=0，
 * 或移除本模块及 src/index.ts、src/cli/commands/serve.ts 中的 startTunnelReadyWatcher 与 chat-ws 的 tunnel_ready 相关逻辑。
 *
 * ICE_TUNNEL_WS_NOTIFY=0 — 关闭探测与推送。
 * ICE_TUNNEL_PROBE_MS — 轮询间隔（毫秒），默认 2500。
 * ICE_TUNNEL_METRICS_PORT / ICE_TUNNEL_METRICS_QUICKTUNNEL — 与 cloudflared --metrics 保持一致（避免 20241 占用冲突）。
 *
 * 未取得公网 URL 时最长探测时间固定为 10 分钟（写死）；超时停止——再晚就绪对当前使用已无意义。
 */

import { fetchQuickTunnelPublicUrl } from './quicktunnel-url.js';

/** 隧道 metrics 轮询最长持续时间（毫秒），超时则停止探测。
 * 10 分钟内 metrics 仍无 URL 视为本轮不可用；延后就绪对用户会话已无意义，不再继续轮询。 */
