...
| L6 | `implement-game-*` | 1+ | 从零实现可玩前端/游戏（Phaser 等），含 E2E + 素材审计 | 中（逻辑高 + 视觉 Judge） |
| **L4+** | `debug-billing-*` | 1 | **97 文件 · ~453KB · 19 跨模块逻辑 BUG**（长上下文 stress） | 高 |
| **L7** | `debug-fusion-*` | 1 | **142 文件 · ~720KB · 28 BUG + 4 空壳 + 5 集成**（融合极难） | 高 |
| **L8** | `debug-saas-order-supply-approval-fusion-*` | 1+ | 企业 SaaS 订单、供应链、审批、计费、审计、迁移与 outbox 融合修复 | 高 |

**合计 16+ 任务**；首批可只跑 L1+L2（8 任务）做 smoke。  
- L6 首发：[`implement-spellbrigade-survivor-01`](../repos/implement-spellbrigade-survivor-01/docs/REQUIREMENTS.md)（时间盒 **240 min** / **200 轮**）。  
- **L4+ 超高难**：[`debug-billing-settlement-03`](../md/debug-billing-settlement-03-任务规格.md)（时间盒 **180 min** / **200 轮**，源码 **~453KB**，显著超出 200K token 上下文）。  
- **L7 极难**：[`debug-fusion-supply-fintech-04`](../md/debug-fusion-supply-fintech-04-任务规格.md)（时间盒 **360 min** / **300 轮**，**33 探针**，四份矛盾文档，单元绿≠集成绿）。  
- **L8 首发**：[`debug-saas-order-supply-approval-fusion-05`](../md/debug-saas-order-supply-approval-fusion-05-任务规格.md)（时间盒 **420 min** / **320 轮**；公开验收全绿 ≠ 隐藏语义探针全绿；评测报告见 [`../reports/debug-saas-order-supply-approval-fusion-05.md`](../reports/debug-saas-order-supply-approval-fusion-05.md)）。
